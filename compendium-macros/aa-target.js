...
if (actionTargets.length === 1) {
  ...
  const adapter = game.modules.get('eskie-macros')?.api?.adapter;
  const distance = adapter ? adapter.getDistance(actionSource, actionTargets) : (canvas.grid.measurePath([actionSource, actionTargets]).euclidean ?? 0);
