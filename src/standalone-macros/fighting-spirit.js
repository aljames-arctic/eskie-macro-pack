// Standalone Macro: Fighting Spirit
// Original Author: .eskie

if (!game.modules.get("sequencer")?.active) {
    return ui.notifications.error("The 'Fighting Spirit' macro requires the 'Sequencer' module to be installed and active!");
}
...
const id = "Fighting Spirit";
const tokenName = token.name ?? "Token";
const label = `${id} - ${tokenName}`;
const labelAlt = `${id} ${tokenName}`;
...
const activeEffects = [
    ...
    ...Sequencer.EffectManager.getEffects({ name: labelAlt, object: token }),
    ...Sequencer.EffectManager.getEffects({ name: id, object: token })
];

if (activeEffects.length > 0) {
    Sequencer.EffectManager.endEffects({ name: label, object: token });
    Sequencer.EffectManager.endEffects({ name: labelAlt, object: token });
